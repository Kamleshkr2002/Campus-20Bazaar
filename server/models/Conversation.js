...
conversationSchema.methods.removeParticipant = function(userId) {
  ...
  this.readStatus = this.readStatus.filter(status => !status.user.equals(userId));
  return this.save();
};

// Method to mark as read for a user
conversationSchema.methods.markAsRead = function(userId) {
  const userReadStatus = this.readStatus.find(status => status.user.equals(userId));
  if (userReadStatus) {
...
conversationSchema.statics.findOrCreate = async function(participants, itemId) {
