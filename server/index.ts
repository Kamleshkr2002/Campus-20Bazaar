  ...
  setupMiddleware() {
    ...
    this.app.get("/health", (req, res) => {
      res.json({
        ...
        onlineUsers: this.chatSocket
          ? this.chatSocket.getOnlineUsersCount()
          : 0,
      });
    });
  }
  ...
  async start() {
    try {
      ...
      this.server.listen(config.port, () => {
        ...
        logger.info(
          `💬 Chat: ${config.features.enableChat ? "Enabled" : "Disabled"}`,
        );
        logger.info(
          `📁 File Upload: ${config.features.enableFileUpload ? "Enabled" : "Disabled"}`,
        );
        ...
        if (config.nodeEnv === "development") {
          ...
          logger.info(
            `💻 Frontend available at: http://localhost:${config.port}`,
          );
        }
      });
